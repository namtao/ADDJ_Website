...
namespace ADDJ.Core.Provider
{
    public abstract class BaseImpl<T> where T : new()
    {
        ...
        public virtual void AddArchiveWithBulk(DataRow[] dtInsert, List<string> param)
        {
        ...
        public virtual int DeleteDynamic(string whereClause)
        {
            ...
            {
                Utility.LogEvent("DeleteDynamic " + TableName + ":ERROR SQL Exception:", se);
                throw new Exception(Constant.MESSAGE_SERVER_QUA_TAI, se);
            }
            catch (Exception ex)
            {
        ...
        public DataTable GetColumnSchema()
        {
            ...
            string commandText = string.Format("SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='{0}'", conn.Database);
            var dsCmd = new SqlCommand(commandText, conn) { CommandType = CommandType.Text };
