...
namespace ADDJ.Core
{
    public class HtmlUtility
    {
        ...
        public static string BuildPagerNormal(int totalrecord, int irecordofpage, int pageindex, string param, string className, string classActive, int rshow)
        {
            var sb = new StringBuilder();
            int numberpage;
            ...
            {
                if (pageindex < numberpage - (rshow - 1) && pageindex != 1)
                {
                    loopstart = pageindex - 1;
                    loopend = pageindex + (rshow - 2);
                    iend = true;
                    if (pageindex > 1)
                    {
                        istart = true;
                ...
                {
                    if (numberpage - rshow > 1)
                    {
                        loopstart = numberpage - rshow + 1;
                        istart = true;
                        loopend = numberpage;
                    }
                    else
            ...
            sb.AppendFormat("<div class=\"gn_pages\"><div style='padding-left: 1px; padding-top: 1px; float: left; height: 23px; overflow: hidden;'>");
            if (istart)
            {
                sb.AppendFormat("<a href='?page={0}{1}'>Đầu tiên</a>", 1, param);
            }
            if (pageindex > 1)
                sb.AppendFormat("<a href='?page={0}{1}' >Trang trước</a>", pageindex - 1, param);
            for (var i = loopstart; i <= loopend; i++)
            {
                if (pageindex == i)
                {
                    sb.AppendFormat("<a class='so {0}' href='#'>", classActive);
                }
                else
                {
                    sb.AppendFormat("<a href='?page={0}{1}'>", i, param);
                }
                sb.Append(i);
                sb.Append("</a>");
            }
            ...
            {
                sb.AppendFormat("<a  href='?page={0}{1}' >Trang sau</a>", pageindex + 1, param);
            }
            if (iend)
                sb.AppendFormat("<a href='?page={0}{1}'>Trang cuối</a>", numberpage, param);
        ...
        public static string BuildPagerSunLaw(int totalrecord, int recordPerPage, int pageIndex, string className, string classActive, int rshow, string catEName, string catType, System.Globalization.CultureInfo cultureInfo)
        {
            ...
            var istart = false;
            var iend = false;
            string strLang = cultureInfo.Name.Substring(0, 2);
            ...
            if (pageIndex > 0)
            {
                sb.Append("<a title='to First Page' href='/" + strLang + "/" + catType + "/1/" + catEName + ".aspx'><<</a>");
                sb.AppendFormat("<a title='Back to Page {0}' href='/" + strLang + "/" + catType + "/{1}/" + catEName + ".aspx'><</a>", pageIndex, pageIndex);
            }
            ...
            for (var i = loopstart; i < loopend; i++)
            {
                ...
                {
                    sb.AppendFormat("<a title='Show result {1} to {2} of {3}' href='/" + strLang + "/" + catType + "/{4}/" + catEName + ".aspx'>", classActive, i * recordPerPage + 1, (i + 1) * recordPerPage, totalrecord, i + 1);
                }
                sb.Append((i + 1).ToString());
                sb.Append("</a>");
            }
            ...
            if ((loopend) < numberpage)
            {
                sb.AppendFormat("<a title='Next to Page {0}' href='/" + strLang + "/" + catType + "/{1}/" + catEName + ".aspx'>></a>", pageIndex + 2, pageIndex + 2);
                sb.AppendFormat("<a title='to Last Page' href='/" + strLang + "/" + catType + "/{0}/" + catEName + ".aspx'>>></a>", numberpage);
            }
        ...
        public static string BuildPagerSunLawSearch(int totalrecord, int recordPerPage, int pageIndex, string className, string classActive, int rshow, string catEName, string catType, System.Globalization.CultureInfo cultureInfo)
        {
            ...
            if (pageIndex > 0)
            {
                sb.Append("<a title='to First Page' href='/" + strLang + "/" + catType + "?page=1&word=" + catEName + "'><<</a>");
        ...
        public static string BuildPager(int totalrecord, int irecordofpage, int pageindex, string divData, string className, string classActive, string url, string typeparam, int rshow)
        {
            ...
            if (istart)
            {
                sb.AppendFormat("<a  href='javascript:void(0)' onclick=\"PagerCommon('{0}','{1}',{2},'{3}'{4})\" ><<Đầu</a> ", url, xparam, 1, divData, jscallback);
            }
            //if (pageindex >= 2)
            //    sb.AppendFormat("<a href='{0}'><Lùi </a>", string.Format(url, pageindex - 1));
            for (var i = loopstart; i < loopend + 1; i++)
            {
                ...
                {
                    sb.AppendFormat("<a href='javascript:void(0)' onclick=\"PagerCommon('{0}','{1}',{2},'{3}'{4})\">{2}</a> ", url, xparam, i, divData, jscallback);
                }
                if (i != loopend)
                    sb.Append(" | ");
            }
            ...
            //{
            //    sb.AppendFormat("<a href='{0}' >Tiếp> </a>", string.Format(url, pageindex + 1));
            //}
            if (iend)
                sb.AppendFormat("<a href='javascript:void(0)' onclick=\"PagerCommon('{0}','{1}',{2},'{3}'{4})\">Cuối>></a>", url, xparam, numberpage, divData, jscallback);
