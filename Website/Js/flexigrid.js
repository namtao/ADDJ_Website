...
(function ($) {
	...
	$.addFlex = function (t, p) {
		...
		p = $.extend({ //apply default properties
		    ...
		    useStatusBar: false, //use status bar
			rp: 15, //results per page
			rpOptions: [10, 15, 20, 30, 50], //allowed per-page values
			title: false,
			idProperty: 'id',
			pagestat: 'Số lượng kết quả từ {from} đến {to} của {total} bản ghi',
			pagetext: 'Trang',
			outof: 'Tổng số',
			findtext: 'Tìm kiếm',
			params: [], //allow optional parameters to be passed around
			procmsg: 'Đang lấy dữ liệu, xin đợi ...',
			query: '',
			qtype: '',
			beforeSend: function () {
			},
		...
		var g = {
			...
			rePosDrag: function () {
				...
				$('thead tr:first th:visible', this.hDiv).each(function () {
				    ...
				    //Longlx Edit
					$('div:eq(' + n + ')', g.cDrag).css({
					    'left': (!(browser.mozilla) ? cdpos - cdcounter + changeSizeDrag : cdpos) + 'px'
					}).show();
					changeSizeDrag++;
				    //End Longlx
					cdleft = cdpos;
			...
			dragEnd: function () {
				...
				if (this.colresize) {
                    ...
                    //Longlx
                    //Viết hàm update file XML
				    //console.log(this.dcoln);
				    //console.log(this.dcolt);
				    if (p.useUpdateCol)
				    {
				        p.callFunctionUpdateColumn(this.dcoln, this.dcolt);
				    }
			...
			buildpager: function () { //rebuild pager based on new properties
			    ...
			    else if (p.useStatusBar)
			    {
			        $('.pPageStat', this.pDiv).html(p.pagestat);
			    }
			},
			fnGetUseUpdateCol: function () {
			    return p.useUpdateCol;
			},
			...
			addCellProp: function () {
				$('tbody tr td', g.bDiv).each(function () {
					...
					if (pth != null) {
						...
						$(tdDiv).css({
							textAlign: pth.align,
							width: (parseInt($('div:first', pth)[0].style.width) + 2)
						});
						if (pth.hidden) {
		...
		if (p.usepager) {
			...
			if (p.searchitems) {
				...
				$(g.sDiv).append("<div class='sDiv2'>" + p.findtext +
						" <input type='text' value='" + p.query + "' size='30' name='q' id='" + p.searchitemidtextbox + "' class='qsbox' /> " +
						" <select name='qtype' id='" + p.searchitemid + "'>" + sopt + "</select>&nbsp;&nbsp;<input id='btnFlexGridSearch' name='btnFlexGridSearch' type='button' value='Tìm kiếm' /></div>");
			    //Split into separate selectors because of bug in jQuery 1.3.2
				$('#btnFlexGridSearch', g.sDiv).click(function () {
				    g.doSearch();
				});
