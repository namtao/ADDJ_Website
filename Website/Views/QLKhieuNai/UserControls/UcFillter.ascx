<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="UcFillter.ascx.cs" Inherits="Website.Views.QLKhieuNai.UserControls.UcFillter" %>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        LinhVucConId_Filter = decodeURIComponent(Utility.GetUrlParam("LinhVucConId")); //Utility.GetUrlParam("LinhVucConId");  

        //console.log(LinhVucConId_Filter);
        //console.log("Test");
        //console.log(encodeURIComponent(Utility.GetUrlParam("LinhVucConId")));
        ...
        $("#txtNgayQuaHanPB_From").datepick({ dateFormat: 'dd/mm/yyyy' });
        $("#txtNgayQuaHanPB_To").datepick({ dateFormat: 'dd/mm/yyyy' });
        ...
        $.getJSON('/Views/QLKhieuNai/Handler/Handler.ashx?key=29&linhVucChungId=-1', '', function (result) {
            if (result != '') {
                ...
                if ((LoaiKNId_Filter == "" || LoaiKNId_Filter == "-1") && (LinhVucChungId_Filter == "" || LinhVucChungId_Filter == "-1")
                    && (LinhVucConId_Filter != "" && LinhVucConId_Filter != "-1")) {
    ...
    function SetParam() {
        ...
        var QHPBTu = Utility.GetUrlParam("QHPBTu");
        if (QHPBTu != "-1" && QHPBTu.length == 8) {
            $("#txtNgayQuaHanPB_From").val(QHPBTu.substring(6, 8) + '/' + QHPBTu.substring(4, 6) + '/' + QHPBTu.substring(0, 4));
        }

        var QHPBDen = Utility.GetUrlParam("QHPBDen");
        if (QHPBDen != "-1" && QHPBDen.length == 8) {
            $("#txtNgayQuaHanPB_To").val(QHPBDen.substring(6, 8) + '/' + QHPBDen.substring(4, 6) + '/' + QHPBDen.substring(0, 4));
        }
    ...
    function Validate() {
        ...
        /*var txtNgayQuaHanPB_From = $("#txtNgayQuaHanPB_From").val();
        var txtNgayQuaHanPB_To = $("#txtNgayQuaHanPB_To").val();
        if (txtNgayQuaHanPB_From != "" && txtNgayQuaHanPB_To == "") {
            value = "Vui lòng chọn ngày quá hạn phòng ban (đến ngảy)";
        } else if (txtNgayQuaHanPB_From == "" && txtNgayQuaHanPB_To != "") {
            value = "Vui lòng chọn ngày quá hạn phòng ban (từ ngày)";
        } else {
            var batdau = txtNgayQuaHanPB_From.split('/');
            var ketthuc = txtNgayQuaHanPB_To.split('/');
            var ngaybatdau = batdau[1] + "/" + batdau[0] + "/" + batdau[2];
            var ngayketthuc = ketthuc[1] + "/" + ketthuc[0] + "/" + ketthuc[2];
            if ((new Date(ngaybatdau).getTime() > new Date(ngayketthuc).getTime())) {
                value = "Ngày quá hạn phòng ban:Từ ngày phải nhỏ hơn đến ngày";
            }
        }*/
    ...
    function fnClearFilter() {
        ...
        $('#txtNgayQuaHanPB_From').val('');
        $('#txtNgayQuaHanPB_To').val('');
        $('#txtNguoiTiepNhan').val('');
        $('#txtNguoiXuLy').val('');
    ...
    function fnLoadDropTrangThai() {
        $.getJSON('/Views/QLKhieuNai/Handler/Handler.ashx?key=35', '', function (result) {
            if (result != '') {
        ...
        $.getJSON('/Views/QLKhieuNai/Handler/Handler.ashx?key=25', '', function (result) {
            if (result != '') {
                ...
                if (LoaiKNId_Filter != "" && LoaiKNId_Filter != "-1") {
                    $("#DropLoaiKhieuNai").val(LoaiKNId_Filter);
                    fnDropLoaiKhieuNaiChange();
                    if (LinhVucChungId_Filter == "" || LinhVucChungId_Filter == "-1")
                        fnLocKhieuNai();
                }
            }
            ...
            <td>
                <table width="100%" border="0">
                    <tbody>
                        <tr>
                            <td style="text-align: left; width: 190px;">
                                <div class="inputstyle">
                                    <div class="bg">
                                        <input id="contentSeach" placeholder="Nhập giá trị tìm kiếm..." style="width: 190px; height: 25px;" />
                                    </div>
                                </div>
                            </td>
                        ...
                        <tr>
                            ...
                            <td style="text-align: left; vertical-align: top" rowspan="2">
                                <div id="divShowCaNhan">
                                    <input id="chkShowCaNhan" type="checkbox" checked="checked" value="1" title="Ẩn hiện khiếu nại đã có người xử lý trong phòng ban" />
                                    <b>&nbsp;&nbsp;&nbsp;Ẩn KN đã có người xử lý trong phòng ban</b>
                                </div>
                            </td>
                        </tr>
                        <tr>
                            <td style="text-align: right">Ngày quá hạn toàn trình
                            </td>
                            <td style="text-align: left">
                                <div class="selectstyle">
                                    <div class="bg">
                        ...
                        <tr>
                            <td style="text-align: right">Ngày quá hạn phòng ban
                            </td>
                            <td style="text-align: left">
                                <div class="selectstyle">
                                    <div class="bg">
                                        <input id="txtNgayQuaHanPB_From" placeholder="Từ ngày..." value="Từ ngày..." style="width: 188px; height: 25px;" />
                                        <input id="txtNgayQuaHanPB_To" placeholder="Đến ngày..." value="Đến ngày..." style="width: 188px; height: 25px;" />
                                    </div>
                                </div>
                            </td>
                        </tr>
