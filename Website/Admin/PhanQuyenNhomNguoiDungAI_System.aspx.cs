...
public partial class admin_PhanQuyenNhomNguoiDungAI_System : PageBase
{
    ...
    private void BindDropDownList()
    {
        try
        {
            var dt = ServiceFactory.GetInstanceNhomNguoiDung_AI().GetList();

            dt.Insert(0, new NhomNguoiDung_AIInfo() { Id = 0, Name = "[ Chọn Nhóm ]" });
    ...
    private void BindGrid(int type)
    {
        try
        {
            var obj = ServiceFactory.GetInstanceNhomNguoiDung_AI_UserRight();
        ...
        try
        {
            ...
            foreach (GridViewRow row in grvView.Rows)
            {
                ...
                if (chkRead.Enabled)
                {
                    ...
                    NhomNguoiDung_AI_UserRightInfo item = new NhomNguoiDung_AI_UserRightInfo();

                    item.MenuID = menuId;

                    var _adminID = int.Parse(ddlUser.Text.ToString());
                    item.NhomNguoiDung_AIId = _adminID;
                    ...
                    var checkExists = obj.GetRightByMenuAndGroupAdmin(item.MenuID, item.NhomNguoiDung_AIId);
                    if (checkExists != null)
                    {
