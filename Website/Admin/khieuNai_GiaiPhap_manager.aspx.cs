...
public partial class admin_khieuNai_GiaiPhap_manager : Page
{
	...
	private void BindGrid(int _khieuNaiId)
	{
		try
		{
			var khieuNai_GiaiPhapObj = ServiceFactory.GetInstanceKhieuNai_GiaiPhap().GetList();
			if (khieuNai_GiaiPhapObj != null && khieuNai_GiaiPhapObj.Count > 0)
			{
				ltThongBao.Text = "<font color='red'>Có " + khieuNai_GiaiPhapObj.Count + " KhieuNai_GiaiPhap được tìm thấy.</font>";
				grvView.DataSource = khieuNai_GiaiPhapObj;
				grvView.DataBind();
			}
	...
	private void RowDataBound(GridViewRowEventArgs e)
	{
		if (e.Row.DataItemIndex != -1)
		{
			...
			e.Row.Cells[1].Text = "<a href='khieuNai_GiaiPhap_add.aspx?ID=" + grvView.DataKeys[e.Row.RowIndex].Value + "'>" + e.Row.Cells[1].Text + "</a>";
		}
	}
	...
	{
		Response.Redirect("khieuNai_GiaiPhap_add.aspx", false);
	}
}
