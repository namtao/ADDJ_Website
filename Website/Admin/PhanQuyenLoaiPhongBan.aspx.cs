...
public partial class admin_PhanQuyenLoaiPhongBan : PageBase
{
    ...
    private void BindDropDownList()
    {
        ...
        try
        {
            ...
            var dt = ServiceFactory.GetInstanceLoaiPhongBan().GetListDynamic("Id,Name","","");

            dt.Insert(0, new LoaiPhongBanInfo() { Id = 0, Name = "[ Chọn Phòng Ban ]" });
            ...
            {
                ddlUser.SelectedValue = Request.QueryString["LoaiPhongBanId"];
            }
        }
        catch
        {
    ...
    private void BindGrid()
    {
        try
        {
            string selectClause = "isnull(a.Id,0) Id, b.Id PermissionId, b.Name, b.MoTa,ISNULL(a.PermissionSchemeId,0) PermissionSchemeId, isnull(a.LoaiPhongBanId,0) LoaiPhongBanId, isnull(a.IsAllow ,0) IsAllow";

            string joinClause = "right join PermissionSchemes b on a.PermissionSchemeId = b.Id and a.LoaiPhongBanId = " + ddlUser.SelectedValue;

            var _dtRight = ServiceFactory.GetInstanceLoaiPhongBan_Permission().GetListDynamicJoin(selectClause, joinClause, "", "b.Sort");
    ...
    protected void linkbtnPhanQuyen_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            int i = 0;
            var obj = ServiceFactory.GetInstanceLoaiPhongBan_Permission();
            ...
            foreach (GridViewRow row in grvView.Rows)
            {
                ...
                LoaiPhongBan_PermissionInfo item = new LoaiPhongBan_PermissionInfo();

                item.IsAllow = true;

                item.LoaiPhongBanId = int.Parse(ddlUser.SelectedValue);
                item.PermissionSchemeId = menuId;
                item.IsAllow = chkRead.Checked;

                var checkExists = obj.GetListByAllRef(item.LoaiPhongBanId, item.PermissionSchemeId);
