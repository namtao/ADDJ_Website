...
public partial class admin_khieuNai_Activity_manager : Page
{
	...
	private void BindGrid(int _khieuNaiId)
	{
		try
		{
			var khieuNai_ActivityObj = ServiceFactory.GetInstanceKhieuNai_Activity().GetList();
			if (khieuNai_ActivityObj != null && khieuNai_ActivityObj.Count > 0)
			{
				ltThongBao.Text = "<font color='red'>Có " + khieuNai_ActivityObj.Count + " KhieuNai_Activity được tìm thấy.</font>";
				grvView.DataSource = khieuNai_ActivityObj;
				grvView.DataBind();
			}
	...
	private void RowDataBound(GridViewRowEventArgs e)
	{
		if (e.Row.DataItemIndex != -1)
		{
			...
			e.Row.Cells[1].Text = "<a href='khieuNai_Activity_add.aspx?ID=" + grvView.DataKeys[e.Row.RowIndex].Value + "'>" + e.Row.Cells[1].Text + "</a>";
		}
	}
	...
	{
		Response.Redirect("khieuNai_Activity_add.aspx", false);
	}
}
