...
public partial class admin_khieuNai_BuocXuLy_manager : Page
{
	...
	private void BindGrid(int _khieuNaiId)
	{
		try
		{
			var khieuNai_BuocXuLyObj = ServiceFactory.GetInstanceKhieuNai_BuocXuLy().GetList();
			if (khieuNai_BuocXuLyObj != null && khieuNai_BuocXuLyObj.Count > 0)
			{
				ltThongBao.Text = "<font color='red'>Có " + khieuNai_BuocXuLyObj.Count + " KhieuNai_BuocXuLy được tìm thấy.</font>";
				grvView.DataSource = khieuNai_BuocXuLyObj;
				grvView.DataBind();
			}
	...
	private void RowDataBound(GridViewRowEventArgs e)
	{
		if (e.Row.DataItemIndex != -1)
		{
			...
			e.Row.Cells[1].Text = "<a href='khieuNai_BuocXuLy_add.aspx?ID=" + grvView.DataKeys[e.Row.RowIndex].Value + "'>" + e.Row.Cells[1].Text + "</a>";
		}
	}
	...
	{
		Response.Redirect("khieuNai_BuocXuLy_add.aspx", false);
	}
}
