...
public partial class admin_dauSoCP_manager : PageBase
{
    ...
    private void BindGrid()
    {
        try
        {
            var dauSoCPObj = ServiceFactory.GetInstanceDauSoCP().GetList();
            if (dauSoCPObj != null && dauSoCPObj.Count > 0)
            {
                ltThongBao.Text = "<font color='red'>Có " + dauSoCPObj.Count + " đầu số CP được tìm thấy.</font>";
                grvView.DataSource = dauSoCPObj;
                grvView.DataBind();
            }
            else
            {
                ltThongBao.Text = "<font color='red'>Không có đầu số CP được tìm thấy.</font>";
    ...
    private void RowDataBound(GridViewRowEventArgs e)
    {
        if (e.Row.DataItemIndex != -1)
        {
            ...
            e.Row.Cells[1].Text = "<a href='dauSoCP_add.aspx?ID=" + grvView.DataKeys[e.Row.RowIndex].Value + "'>" + e.Row.Cells[1].Text + "</a>";
        }
    }
    ...
    {
        Response.Redirect("dauSoCP_add.aspx", false);
    }
