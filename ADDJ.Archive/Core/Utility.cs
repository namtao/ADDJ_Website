...
namespace GQKN.Archive.Core
{
    public class Utility
    {
        ...
        {
            string path = string.Empty; // Folder ghi log
            // Nếu là web
            ...
            if (!path.EndsWith("\\")) path = string.Concat(path, "\\", string.Format("Logs\\"));
            ...
            string pathFolder = string.Concat(path, string.Concat(time, "\\"));
        ...
        public static void LogEventService(string message, EventLogEntryType type)
        {
            ...
            path = string.Concat(path, string.Concat(time, "\\"));
